/*
 *
 * https://leetcode.com/problems/ipo
 *
 * # 502. IPO
 *
 *   Q. Suppose LeetCode will start its IPO soon. In order to sell a good price of its shares to Venture Capital, LeetCode
 *      would like to work on some projects to increase its capital before the IPO. Since it has limited resources, it can
 *      only finish at most k distinct projects before the IPO. Help LeetCode design the best way to maximize its total
 *      capital after finishing at most k distinct projects.
 *
 *      You are given n projects where the ith project has a pure profit profits[i] and a minimum capital of capital[i] is
 *      needed to start it.
 *
 *      Initially, you have w capital. When you finish a project, you will obtain its pure profit and the profit will be
 *      added to your total capital.
 *
 *      Pick a list of at most k distinct projects from given projects to maximize your final capital, and return the final
 *      maximized capital.
 *   Ex.
 *      Input : k = 2, w = 0, profits = [1, 2, 3], capital = [0, 1, 1]
 *      Output: 4
 *      Explanation: Since your initial capital is 0, you can only start the project indexed 0.
 *                   After finishing it you will obtain profit 1 and your capital becomes 1.
 *                   With capital 1, you can either start the project indexed 1 or the project indexed 2.
 *                   Since you can choose at most 2 projects, you need to finish the project indexed 2 to get the maximum capital.
 *                   Therefore, output the final maximized capital, which is 0 + 1 + 3 = 4.
 *
 *  Constraints:
 *          1 <= k <= 10⁵
 *          0 <= w <= 10⁹
 *          n == profits.length
 *          n == capital.length
 *          1 <= n <= 10⁵
 *          0 <= profits[i] <= 10⁴
 *          0 <= capital[i] <= 10⁹
 */

import java.util.*;

public class Day_15_LC_502 {

    /// main Method
    public static void main(String[] args) {
        Scanner sc = new Scanner(System.in);

        System.out.print("K: ");
        int k = sc.nextInt();

        System.out.print("W: ");
        int w = sc.nextInt();

        sc.nextLine();

        System.out.println("Enter profits: ");
        String[] p = sc.nextLine().split(" ");

        System.out.println("Enter capitals: ");
        String[] c =  sc.nextLine().split(" ");

        int n = p.length;
        if (n != c.length) return;
        int[] profits = new int[n];
        int[] capitals = new int[n];

        for (int i = 0; i < n; i++) {
            profits[i] = Integer.parseInt(p[i]);
            capitals[i] = Integer.parseInt(c[i]);
        }

        System.out.println("Final capital after maximizing: ");
        System.out.println(findMaximizedCapital(k, w, profits, capitals));
    }

    /// Solution
    static int findMaximizedCapital(int k, int w, int[] profits, int[] capital) {
        int n = profits.length;
        int[][] capitalProfits = new int[n][2];
        PriorityQueue<Integer> pq = new PriorityQueue<>(Collections.reverseOrder());

        for (int i = 0; i < n; i++) {
            capitalProfits[i][0] = capital[i];
            capitalProfits[i][1] = profits[i];
        }

        Arrays.sort(capitalProfits, Comparator.comparingInt(cp -> cp[0]));

        int idx = 0;
        for (int i = 0; i < k; i++) {

            // adding the profits which required capital is smaller than my current capital
            while (idx < n && capitalProfits[idx][0] <= w) {
                pq.offer(capitalProfits[idx][1]);
                idx++;
            }

            if (pq.isEmpty()) break;

            w += pq.poll();
        }

        return w;
    }
}